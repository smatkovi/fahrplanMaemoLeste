...
void QValueButton::paintEvent(QPaintEvent *)
{
    ...
    int vOffset = isDown() ? (int)QStyle::PM_ButtonShiftVertical : 0;
    int hOffset = isDown() ? (int)QStyle::PM_ButtonShiftHorizontal : 0;
    ...
    int descriptionWidth = descriptionFm.size(Qt::TextSingleLine, description().section('@',0,0)).width();
    int textHeight = textFm.size(Qt::TextSingleLine, text()).height();
    int descriptionHeight =descriptionFm.size(Qt::TextSingleLine, description().section('@',0,0)).height();
    ...
    p.drawItemText(descriptionRect().translated(hOffset + (titleHOffset + (deltaText < 0 ? 0 : deltaText)), vOffset + titleVOffset), textflags,
                    pal, isEnabled(), description().section('@',0,0), QPalette::ButtonText);
    p.restore();
}
