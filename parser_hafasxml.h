...
private:
    QList<ResultItem> lastItems;
    QBuffer *filebuffer;
    QString cleanHafasDate(QString time);
    QDateTime cleanHafasDateTime(QString time, QDate date);
    ResultInfo parseJourneyData(QByteArray data, bool append);
    DetailResultInfo parseJourneyDataDetails(QByteArray data, QString connectionId, QDate date);
};
